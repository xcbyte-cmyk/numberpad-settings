...
`/` → Tab, `*` → 왼쪽 Shift, `-` → 오른쪽 Shift 등은 Mac용 규칙으로 구성합니다. `0`의 앱샷 연결은 해당 Mac의 Codex 버전·단축키·실제 첨부 동작을 확인해야 합니다. 이 저장소는 Mac에서 동작 검증된 매핑을 제공하지 않습니다.

## 검증 범위

- 원래 Windows PC에서 0번의 실제 앱샷 실행을 사용자가 확인했습니다.
- 내보낸 15개 키 설정과 매크로 2개가 원본과 일치함을 확인했습니다.
