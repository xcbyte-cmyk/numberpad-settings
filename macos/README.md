...
1번은 `F13`, 2번은 `F14`, 3번은 `F15`를 전송합니다. Codex의 대응 단축키도 같은 키로 설정합니다. macOS 시스템 설정 → 키보드 → 키보드 단축키 → 디스플레이에서 `F14` 밝기 감소와 `F15` 밝기 증가를 해제해야 충돌을 피할 수 있습니다. MonitorControl을 사용한다면 해당 앱의 F14/F15 밝기 제어도 해제합니다. 이 Mac에서는 두 밝기 단축키를 해제했으며, Codex의 2·3번 실제 기능 동작은 추가 확인이 필요합니다.

계산기 버튼은 `Microsoft Number Pad — Accessibility Keyboard` 단축어로 손쉬운 사용 키보드를 엽니다. 점 키는 `0909` 입력 후 Enter를 보냅니다. Caps Lock이 켜져 있어도 점 키 규칙이 적용되도록 `from.modifiers.optional`에 `caps_lock`을 포함합니다. 다른 맥에는 같은 이름의 단축어를 별도로 준비해야 합니다. 6번 받아쓰기는 Karabiner에서 지원하는 `consumer_key_code: dictation`을 사용합니다.
...
설정 JSON 저장과 Karabiner 드라이버 연결·가상 키보드 준비 상태는 확인했습니다. 점 키 수정 후 사용자 입력에 `0909`가 나타난 것은 확인했으며, Enter 이벤트는 별도로 검증하지 않았습니다. 실제 4·5·6번 음성 동작과 0번 앱샷 동작은 아직 확인하지 않았습니다. 기존 설정으로 복구하려면 적용 전 백업을 사용하세요.
